...
class ProtolanguageSimulation:
    def __init__(self, n_pop=100, n_objects=5, n_signals=5, sample_size=15):
        ...
        for _ in range(self.n_pop):
            P = self._normalize_rows(np.random.rand(n_objects, n_signals))
            Q = self._normalize_rows(np.random.rand(n_signals, n_objects))
    ...
    def reproduce_and_sample(self, parent_matrices):
        ...
        return {'P': self._normalize_rows(child_P), 'Q': self._normalize_rows(child_Q)}
